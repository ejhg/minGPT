...
class AdditionDataset(Dataset):
    ...
    At tests time, we will feed in an addition problem by giving the first 2n digits,
    ...
    def __init__(self, config, split):
        self.config = config
        self.split = split  # train/tests

        # split up all addition problems into either training data or tests data
        ...
        self.ixes = perm[:num_test] if split == 'tests' else perm[num_test:]
...
class TestAdder(unittest.TestCase):
    ...
    def test(self):
        ...
        test_dataset = AdditionDataset(config.data, split='tests')
        ...
        def eval_split(trainer, split, max_batches=None):
            dataset = {'train': train_dataset, 'tests': test_dataset}[split]
        ...
        with torch.no_grad():
            ...
            test_score = eval_split(trainer, 'tests', max_batches=None)

        self.assertGreaterEqual(train_score, 0.9)
        self.assertGreaterEqual(test_score, 0.9)
